    ...
    return(
        <>
            {Paras.map(para => <div className='w-[26.66%]' key={para.id}>
                                    <p className='text-center text-amber-600 text-[26px] font-semibold underline uppercase'>{para.firstText}</p>
                                    <p className='text-center text-black text-xl font-normal'>{para.secondText}</p>
                                </div>) }
        </>
    )
}